...
// 각 timer tick마다 timer interrupt handler에 의해서 호출된다.
void thread_tick(void) {
  ...
  /* Enforce preemption. */
  // 이때 thread_ticks는 shcedule이 된 이후 지난 timer의 tick 수 이다.
  if (++thread_ticks >=
      TIME_SLICE)  // thread_ticks는 맨 처음 schedule()에서 0으로 만들어줌.
...
// 주어진 시간만큼 thread를 잠재운다
// ticks(특정 시간까지 잠재운다) : 부팅 이후 경과된 시간 + 잠재울 시간
...
/**
        다음 schedule될 스레드를 return 한다.

        ready queue에서 선택하며 만약 ready queue가 비어있다면 idle thread가
   리턴된다
*/
static struct thread *next_thread_to_run(void) {
...
// thread를 깨워서 해당 thread를 ready 상태로 변환
void thread_wake(int64_t ticks) {
  // global tick(최소 local tick)이 현 tick보다 더 크다
  // 즉 아직 깨울 thread가 존재하지 않는다
  if (get_global_ticks() > ticks) {
    return;
  }

  // sleep list가 비어있지 않을 때까지 검사한다
  while (!list_empty(&sleep_list)) {
    struct list_elem *target_e = list_begin(&sleep_list);
    struct thread *target_thread = list_entry(target_e, struct thread, elem);

    // 더 이상 깨울 thread가 존재하지 않는다
    if (target_thread->local_tick > ticks) {
      break;
    }

    // thread를 깨운다
    struct list_elem *pop_e = list_pop_front(&sleep_list);
    struct thread *pop_thread = list_entry(pop_e, struct thread, elem);

    enum intr_level old_level = intr_disable();

    // thread : sleep -> ready
    pop_thread->status = THREAD_READY;
    list_insert_ordered(&ready_list, pop_e, compare_thread_local_tick,
                        pop_thread->local_tick);
    intr_set_level(old_level);
  }
  update_global_ticks();
}

// global tick은 sleep list에 있는 thread 중 가장 tick이 작은 것을 뜻한다
void update_global_ticks() {
  if (!list_empty(&sleep_list)) {
    struct list_elem *min_tick_e = list_front(&sleep_list);
    struct thread *min_tick_thread =
        list_entry(min_tick_e, struct thread, elem);

    global_tick = min_tick_thread->local_tick;
  }
}
