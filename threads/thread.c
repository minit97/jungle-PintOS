...
tid_t  // 이름, 우선순위, 실행될 함수, 새 스레드에서 실행될 함수에 전달될 인자
       // -> function은 새 스레드에서 실행할 함수로 auxf를 이용하여 동작 수행
thread_create(const char *name, int priority, thread_func *function,
  ...
  /* Allocate thread. */
  t = palloc_get_page(PAL_ZERO);  // 페이지 할당자를 통해 메모리를 할당하고
                                  // 할당된 메모리를 0으로
...
void thread_block(void) {
  ...
  schedule();  // 스케쥴러를 호출하여 새로운 쓰레드를 선택하고 실행 -> 다음
               // 쓰레드를 선택하기 위해 스케쥴러 호출
}

/* 차단된 스레드 T를 준비-실행 상태로 전환합니다.
   T가 차단되지 않은 상태라면 이는 오류입니다. (실행 중인 스레드를 준비 상태로
   만들기 위해서는 thread_yield()를 사용하세요.)

   이 함수는 실행 중인 스레드를 선점하지 않습니다. 이는 중요할 수 있습니다:
   호출자가 인터럽트를 직접 비활성화한 경우, 스레드를 원자적으로 차단 해제하고
...
void thread_set_nice(
    int nice UNUSED) {  // 현재 스레드의 nice 값을 새 값으로 설정
  enum intr_level old_level = intr_disable();
...
int thread_get_nice(void) {  // 현재 스레드의 nice 값을 반환
  enum intr_level old_level = intr_disable();
...
int thread_get_load_avg(void) {  // 현재 시스템의 load_avg * 100 값을 반환
  enum intr_level old_level = intr_disable();
...
int thread_get_recent_cpu(void) {  // 현재 스레드의 recent_cpu * 100 값을 반환
  enum intr_level old_level = intr_disable();
  int recent_cpu =
...
static void init_thread(struct thread *t, const char *name, int priority) {
  ...
  ASSERT(PRI_MIN <= priority &&
         priority <= PRI_MAX);  // 우선순위가 유효한 범위 내인지
...
/* 스케쥴링 루틴을 수행 현재 실행중인 스레드 상태 변경, 스케쥴러를 호출해서
 * 다음에 실행한 스레드를 선택 후 실행*/
static void do_schedule(int status) {
  ASSERT(intr_get_level() == INTR_OFF);
...
/* 스케쥴러가 다음에 실행한 스레드를 선택하고, 필요한 경우 스레드 컨텍스트
 * 전환*/
static void schedule(void) {
  ...
  if (curr != next) {
    /* If the thread we switched from is dying, destroy its struct
       ...
       schedule(). */
    /* 현재 스레드가 종료 상태고 초기 스레드가 아닌경우 현재 쓰레드를
     * destruction_req에 추가 */
...
void thread_awake(int64_t ticks) {
  ...
  while (!list_empty(&sleep_list)) {
    ...
    struct thread *wakeup_target_thread =
        list_entry(wakeup_target_e, struct thread, elem);
...
bool cmp_sema_priority(struct list_elem *cur, struct list_elem *cmp,
                       void *aux) {
...
void cal_priority(struct thread *cur_thread) {
  if (cur_thread == idle_thread) return;
  // cur_thread->priority = fp_to_int (add_mixed (div_mixed
  // (cur_thread->recent_cpu, -4), PRI_MAX - cur_thread->nice * 2));
  cur_thread->priority =
      PRI_MAX - fp_to_int(cur_thread->recent_cpu / 4) - (cur_thread->nice * 2);
}
...
void cal_decay(void) { return (2 * load_avg) / (2 * load_avg + 1); }

void cal_recent_cpu(struct thread *cur_thread) {
  if (cur_thread == idle_thread) return;
