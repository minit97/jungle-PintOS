...
thread_init (void) {    // Add the code to initialize the sleep queue data structure
    ...
    // mlfqs
    load_avg = 0;       /* At system boot, it is initialized to 0 */
    list_init (&all_list);
...
tid_t thread_create (const char *name, int priority, thread_func *function, void *aux) {
    ...
    t->fdt = palloc_get_multiple(PAL_ZERO, 2);  // 한 페이지당 4096 bytes
...
thread_unblock (struct thread *t) {
    // project1[scheduling] - when the thread unlocked, it is inserted to ready_list in the priority order
...
thread_yield (void) {                                   // yield the cpu and insert thread to ready_list
    // project1[scheduling] - the current thread yields CPU and it is inserted to ready_list in priority order.
...
void mlfqs_load_avg(void) {
    ...
    load_avg_weighted = divide_with_fp(load_avg_weighted, 60);

    // (1/60) * ready_threads 계산
    int ready_threads_weighted = divide_with_fp(ready_threads_fp, 60);
